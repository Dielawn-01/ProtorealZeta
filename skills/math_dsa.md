...
| `associator(u,v,w)` | `(u·v)·w − u·(v·w)` | Curvature at Bridge: κ.a = −1 |

### Little Delta (δ) — The Observer

The dual to operators. Where operators transform, δ *measures*:

```
δ(u) = |ι| · SR(u) = |ι| · (a − ω·ι)
```

- `flip(δ)` = observe from the other side of the bridge: `δ(u*)`
- `scale(δ, k)` = rescale measurement window: `k · δ(u)`
- **ε-δ limit:** For every ε > 0, ∃ δ such that δ(u_n) < ε ⟹ |a_n − L| < ε

The Cauchy limit embeds as the identity observer `δ(u) = a − L`.

### Activation Functions (Sectoral Expansion)

Apply transcendental functions while respecting the 5-component geometry:

```
mesh_f(u) = { a := f(a+r), b := f(a+b+r) − f(a+r), m := f'(a+r)·m, e := f'(a+r)·e, l := f'(a+r)·l }
```

| Function | Core | Derivative Scaling |
|----------|------|--------------------|
| `mesh_tanh` | `tanh(a+r)` | `1 − tanh²(a+r)` |
| `mesh_sinh` | `sinh(a+r)` | `cosh(a+r)` |
| `mesh_sin` | `sin(a+r)` | `cos(a+r)` |

### Conic Sections in the (b, m) Plane

| Conic | Equation | Δ | Meaning |
|-------|----------|---|---------|
| Bridge | bm = 1 | +1 (hyperbola) | Every nonzero fiber |
| Parity | (b−m)² = 0 | 0 (degenerate) | Parity lock b = m |
| Thrust | b²+m² = 2 | −4 (ellipse) | Closed orbit |

**Proven:** Bridge ∧ Parity → b = m = ±1. Monster Inverse maps between conics.
