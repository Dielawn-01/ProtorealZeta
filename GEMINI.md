...
1. **ZERO SORRY POLICY**: This repo has 0 `sorry` across 27 modules. Never introduce `sorry` unless explicitly asked for a stub. If a proof is difficult, use `omega`, `simp`, `ring`, or `decide` tactics first.
...
4. **Testing**: Run `lake build` to verify. All 27 modules must compile without errors or warnings.
